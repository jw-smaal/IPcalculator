...
- (void) testIPplanClassMethods{
	...
	XCTAssertEqual (b, c, @"failed setbits");	
	...
	XCTAssertEqual (b, c, @"failed clearbits");
	...
	XCTAssertEqual (a, b, @"failed bitwitdh");
...
- (void) testIPplanInstanceMethods{
	...
	XCTAssertEqual(a, (uint32_t)512, @"Should be rounded up");
	XCTAssertEqual(b, (uint32_t)8, @"Should be rounded up");	
	...
	XCTAssertEqual (a, b, @"failed ipplan");
